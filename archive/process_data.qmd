...
# Loading dataset
dat_reg <- read.csv("data/all_1020.csv") |> 
...
write.csv(dat_reg, "data/processed_data.csv", row.names = FALSE, na = "")
```
...
sum(dat_reg$fa_vn + dat_reg$fa_jp + dat_reg$fa_o) == nrow(dat_reg)
...
# Check distribution
summary(dat_reg$n_vn_authors)
summary(dat_reg$n_jp_authors)

# Cross-tabulation
table(dat_reg$n_vn_authors, dat_reg$n_jp_authors)
...
sample_row <- sample(1:nrow(dat_reg), 1)
dat_reg[sample_row, c("affiliations", "n_vn_authors", "n_jp_authors")]
```

export

```{r}
sdg_results$document <- as.integer(sdg_results$document)
dat_reg <- dat_reg |>
...
dat_reg <- dat_reg |>
  mutate(
    ...
    across(starts_with("SDG-"), ~ if_else(. > 0, 1, 0)),
