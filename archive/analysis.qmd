...
```{r}
ggplot(dat_reg, aes(x = log1p(cited))) +
  geom_histogram(bins = 21, color = "white") +
  theme_bw()

ggplot(dat_reg, aes(x = n_funder, y = log1p(cited))) +
...
```{r}
# By OA
wilcox.test(cited ~ OA, data = dat_reg)

# By coop
wilcox.test(cited ~ coop, data = dat_reg)

# By fund or not
wilcox.test(cited ~ fund, data = dat_reg)

wilcox.test(cited ~ vn, data = dat_reg)

# By quartile
kruskal.test(cited ~ quartile, data = dat_reg)
pairwise.wilcox.test(dat_reg$cited, dat_reg$quartile,
                     p.adjust.method = "bonferroni")

# By number of funders origins
kruskal.test(cited ~ n_funder, data = dat_reg)
pairwise.wilcox.test(dat_reg$cited, dat_reg$n_funder,
                     p.adjust.method = "bonferroni")

# By number of funders types
kruskal.test(cited ~ n_ftype, data = dat_reg)
pairwise.wilcox.test(dat_reg$cited, dat_reg$n_ftype,
                     p.adjust.method = "bonferroni")
```
...
table_oa <- table(dat_reg$coop, dat_reg$OA)
chisq.test(table_oa)

# ANOVA/Kruskal for SJR
kruskal.test(sjr_score ~ coop, data = dat_reg |> filter(year > 1990, !is.na(sjr_score)))
```
...
nb_model <- glm.nb(cited ~ OA + coop + n_countries + old + quartile + ind + pub + uni + jap + vn, data = dat_regre)
...
```{r}
# Fit the model (assuming dat_regre is prepared)
...
table(dat_reg$fa_vn, dat_reg$fa_jp, dat_reg$fa_o)
prop.table(table(dat_reg$collab_type))

# 2. SDG patterns
top_sdgs <- colSums(select(dat_reg, starts_with("SDG_"))) |> 
  sort(decreasing = TRUE) |> head(5)

# 3. Quality by leadership
dat_reg |> group_by(fa_vn, fa_jp) |> 
  summarise(mean_cited = mean(cited, na.rm=T),
            pct_q1 = mean(quartile=="Q1", na.rm=T))

# 4. Funding impact
dat_reg |> group_by(fund) |> 
  summarise(mean_cited = mean(cited, na.rm=T),
            mean_sdg = mean(n_sdg, na.rm=T))

# 5. Bilateral vs multilateral
dat_reg |> group_by(coop) |> 
  summarise(mean_cited = mean(cited, na.rm=T),
