...
This report analyzes temporal trends in **`r nrow(dat_reg)` journal articles** resulting from Japan-Vietnam research collaborations (1973-2024). Key findings include:
...
annual_output <- dat_reg |> 
  filter(year >= cutoff_year) |> 
  count(year, name = "publications")

# Calculate growth statistics
total_pubs <- nrow(dat_reg)
recent_5yr <- dat_reg |> filter(year >= 2020) |> nrow()
...
if (all(subject_cols %in% names(dat_reg))) {
  diversity_trends <- dat_reg |> 
    filter(year >= cutoff_year) |> 
