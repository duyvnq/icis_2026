...
# Load processed data
dat_reg <- read_csv("/Users/duyvnq/Library/CloudStorage/OneDrive-Personal/Documents/Projects/icis_2026/data/processed_data.csv", show_col_types = FALSE) |> 
...
**Total records**: `r format(nrow(dat_reg), big.mark = ",")` articles

**Total columns**: `r ncol(dat_reg)`
...
### id {-}
**Type**: Integer  
**Description**: Unique row identifier (1 to `r nrow(dat_reg)`)  
**Missing**: `r sum(is.na(dat_reg$id))`

### DOI {-}
**Type**: Character  
**Description**: Digital Object Identifier  
**Missing**: `r sum(is.na(dat_reg$DOI))` (`r round(100*mean(is.na(dat_reg$DOI)), 1)`%)

### year {-}
**Type**: Integer  
**Description**: Publication year  
**Range**: `r min(dat_reg$year, na.rm=TRUE)` – `r max(dat_reg$year, na.rm=TRUE)`  
**Missing**: `r sum(is.na(dat_reg$year))`

```{r year-summary}
summarize_numeric(dat_reg$year) |> 
...
**Unique values**: `r n_distinct(dat_reg$title, na.rm = TRUE)`  
**Missing**: `r sum(is.na(dat_reg$title))`
...
**Range**: `r min(dat_reg$cited, na.rm=TRUE)` – `r max(dat_reg$cited, na.rm=TRUE)`

```{r cited-summary}
summarize_numeric(dat_reg$cited) |> 
...
```{r quartile-summary}
summarize_categorical(dat_reg$quartile) |> 
...
```{r sjr-summary}
summarize_numeric(dat_reg$sjr_score) |> 
...
```{r coop-summary}
summarize_categorical(dat_reg$coop) |> 
...
summarize_numeric(dat_reg$n_countries) |> 
...
```{r fund-summary}
summarize_categorical(dat_reg$fund) |> 
...
dat_reg |> 
  ...
  mutate(
    ...
    Percentage = round(100 * Articles / nrow(dat_reg), 1)
  ) |> 
...
```{r nregions-summary}
summarize_categorical(dat_reg$n_regions) |> 
...
```{r nsectors-summary}
summarize_categorical(dat_reg$n_sectors) |> 
...
```{r prop-vn-summary}
summarize_numeric(dat_reg$prop_vn_authors) |> 
...
```{r collab-type}
summarize_categorical(dat_reg$collab_type) |> 
...
```{r oa-summary1}
summarize_categorical(dat_reg$OA) |> 
...
```{r nsdg-summary}
summarize_numeric(dat_reg$n_sdg) |> 
